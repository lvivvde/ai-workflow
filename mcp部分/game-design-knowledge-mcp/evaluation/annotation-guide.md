...
这条字形缺陷由 issue #28 跟踪并已收口：`flow-arrow-v2` 起，方向不再只信转写，箭头块自己的墨迹必须与转写一致才确认（规则表与实测矩阵见 `docs/layout.md` 的「方向为什么需要像素佐证」）。构造规则因此不变——画成自下而上的 `↑↑` 依旧是被转写与像素同时支持的那一种画法；但若将来把向下箭头画进语料，管线不会再把它记成一条反向的 confirmed 关系，而会记成 `candidate` + `direction_conflict`，标注可以按「方向未证」评估它。
