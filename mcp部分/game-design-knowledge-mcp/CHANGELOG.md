...
- 新增 `tests/test_evaluation_fixtures.py`（13 项）：图片资源的解析、缺图/漂移/越界/非 PNG 的拒绝、占位图这条降级路径仍可用，以及「已提交字节 == 渲染器输出」「渲染器画的行 == 标注的 regions」「`transcription.text` == `regions` 拼接」三条回归。
