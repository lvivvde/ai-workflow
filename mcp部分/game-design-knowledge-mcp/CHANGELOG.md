...
- 新增 `tests/test_evaluation_fixtures.py`（15 项）：图片资源的解析、缺图/漂移/越界/非 PNG/「有 pin 却没有 asset」的拒绝、占位图这条降级路径仍可用（独立 PNG 与 DOCX 内嵌图片各一条），以及「已提交字节 == 渲染器输出」「渲染器画的行 == 标注的 regions」「`transcription.text` == `regions` 拼接」三条回归。
