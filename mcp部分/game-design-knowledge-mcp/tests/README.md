...
- `test_evaluation_fixtures.py`：语料图片必须真的存在、真的是 PNG、不越出语料目录、字节与 `asset_sha256` 一致，缺图 / 漂移 / 越界 / 无语料目录 / 只给 pin 不给 `asset` 都报 `SchemaError`，没有 `asset` 的图片（独立 PNG 与 DOCX 内嵌图片）仍走占位图这条降级路径；另外把「已提交的字节 == 渲染器输出」「渲染器画的行 == 标注的 regions」「`transcription.text` == `regions` 的拼接」钉成回归，防止再出现「标注描述一张像素里不存在的图」。
