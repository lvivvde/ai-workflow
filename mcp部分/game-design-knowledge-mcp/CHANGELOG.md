...
- 修 `structure_relations` 会把箭头**读反**（issue #28）：方向原来只信转写，而本机实测（Windows 11 / RapidOCR 1.3.24，4 种字体 × 5 个字号、30 多种渲染）里画成 `↓↓` 的块会被读成 `↑↑`（转写置信度 0.58–0.87），真 `↑↑` 是 0.77–0.89——两个总体重叠，**任何**置信度阈值都分不开「读对了」和「读反了」，而读反不是噪声：它把整条流程的源与目标对调。规则集升到 **`flow-arrow-v2`**，新增 `src/game_design_knowledge/ink.py`（`arrow-ink-v1`）从块自己的像素量方向：轴仍由转写决定（实测误读都在同一族内，从不跨轴），极性改由「最宽墨迹带在框里的位置」决定（实测 `↑`/`↑↑` 0.16–0.33、`↓`/`↓↓` 0.65–0.82 的框高；`→`/`→→` 0.56–0.80、`←`/`←←` 0.34–0.43 的框宽），带有两个必须一起看的门槛——离框中线不足 `INK_MARGIN=0.10` 就不表态，块形状与转写声称的轴不符也不量（横向块的**纵向** profile 是上重的 0.24–0.46，少了这道门槛就会拿错轴的读数去「确认」自己）。
- `confirmed` 的条件多了一条「方向站得住」，并且每次都说得出它站的理由：`geometry_basis` 现在写出最宽墨迹带落在框里的哪个位置、与转写是否一致。两者相反记 `candidate` + `direction_conflict`（`detail` 明说端点按转写给出、**可能是反的**），这次构建读不到墨迹（图片解不开、块没有边界框、形状与轴不符、框里读不到形状）记 `candidate` + `direction_unverified`——都比「照转写确认」诚实。结构性问题（分叉、跨列、缺端点、连续箭头、重叠）优先级更高，方向的分歧仍会写进 `geometry_basis`。文档与规则表同步到 `docs/layout.md`（新增「方向为什么需要像素佐证」一节与两行 `uncertainty`）。
- `tests/document_fixtures.py` 新增 `arrow_png()`（按行拼字节画「杆 + 越远越宽的三角头」，不依赖绘图库），让索引与流水线测试能喂**真像素**而不是 8x8 纯色占位图：`tests/test_layout_index.py` 与 `tests/test_processing_pipeline.py` 的端到端 fixture 换成它之后，`layout_confirmed_relations` 的 confirmed 关系是「像素 + 转写互相佐证」得来的。新增 `tests/test_ink.py`（10 项，核心是「引擎把向下箭头镜像成 `↑↑` 时像素一个字节都没变」）与 `tests/test_layout.py` 的 3 项方向证据测试（镜像读法只能得到 `direction_conflict` 的 candidate、没有读数只能是 `direction_unverified`、结构问题优先于方向问题）。
- 修 `enhanced_ocr` 的 pin 指向不存在的版本：包声明的 `paddlex==2.4.4` 在索引里没有（只有 2.0.0/2.1.0 与 3.x），闭包因此无法解析、bundle 也无法为它生成哈希 requirements。现在固定到 **PaddleOCR 3.x 代际**（`paddleocr==3.7.0`、`paddlex==3.7.2`、`paddlepaddle==3.3.1`，PP-StructureV3 只存在于这一代），并像 `core` / `visual` 一样声明完整 wheel 闭包：67 个传递依赖，共 70 个 wheel，全部有 win_amd64/cp312 的二进制分发，`--only-binary :all: --require-hashes` 能一次装完。
- 新增 `capabilities/locks/enhanced_ocr-win_amd64-cp312.json`：一次真实解析的记录（顶层 pin、70 个 wheel 的文件名与 SHA256、解析所在的平台与解释器）。清单里的闭包必须与 lock 逐条相同，`tests/test_offline_bundle.py` 新增 3 项测试比对，所以「pin 指向一个索引里不存在的版本」不必等到联网解析才暴露；只带 wheel 不带 sdist 同样由测试保证，因为离线安装用 `--only-binary :all:`。
- 修 PaddleOCR 适配器在真机上**根本跑不起来**的两处，都是装完引擎后才暴露的：语言码（PaddleOCR 没有 `chi`，读中文加拉丁字母的模型是 `ch`；项目按 Tesseract 写法配置语言，现在只翻译文档里支持的语言，其余原样传下去让引擎报出它服务不了哪一门）与 pinned 运行时 `paddlepaddle==3.3.1` 的 oneDNN 路径（对 PP-OCRv4/v5/v6 一律抛 `ConvertPirAttribute2RuntimeAttribute not support [pir::ArrayAttribute<pir::DoubleAttribute>]`，改为 `enable_mkldnn=False` 构造，用一部分 CPU 速度换引擎能跑）。`tests/test_ocr_images.py` 新增 3 项语言码测试。
- `enhanced_ocr` 真机验收记录（Windows 11 / AMD64 / CPython 3.12）：`capability verify --pack enhanced_ocr` 逐条通过；离线 `pip install --no-index --only-binary=:all: --require-hashes` 在干净解释器装完 70 个 wheel（返回码 0，安装后 838 MB，`paddleocr`/`paddlex`/`paddle` 分别导入到 3.7.0/3.7.2/3.3.1）；项目自己的适配器在该解释器里把随仓四张语料图全部转录为 `succeeded`（`战斗流程` 6 个区域、`连招流程` 2 个、`结算流程` 4 个、`结算发奖流程` 2 个）。两条边界同期写进 `docs/capabilities.md` 第 8 节：PaddleX 的官方模型不在包里（实测 7 个模型 192 MB，第一次构造 pipeline 时由引擎自己取，离线机器要预先放好缓存树），以及 `core` 与 `enhanced_ocr` 装进同一个解释器时 `PyYAML`（6.0.2 / 6.0.3）与 `opencv-contrib-python` / `opencv-python` 只能字面满足一方。
- 文档同步新的代际、体积与边界：`docs/capabilities.md`（表格行、lock 说明、验收清单一行、第 8 节改写）、`docs/processing.md`（表格行；「不静默下载」一节写明这条性质管的是本项目，不含引擎自己取模型这一形态）、`docs/ocr.md`（PaddleOCR 适配器的语言码、oneDNN 与模型文件的实测结论）、`capabilities/README.md`（新增 `locks/`）。声明体积按实测改为下载 ~400 MB（wheel 211 MB + 引擎模型 192 MB）、安装 ~1.03 GB。
