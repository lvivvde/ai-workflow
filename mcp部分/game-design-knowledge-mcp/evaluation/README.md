...
| `retrieval` | 已测量：recall@limit、precision、MRR、无答案误报；V2 检索底座已交付（`retrieval-v1`：原查询 + 确定性扩展 + 命名空间隔离通道 + 证据回读 + 冲突保护），可选向量能力也已交付（`semantic-v1`，默认关闭的 `hashing` 参考 provider + 可删除侧车 + 查询改写守门），但 Golden Set 上的向量增益测量仍缺，故按当前语料测量 |
